...
protected:
    ...
    VideoFramePtr getVideo(pts position, IClipPtr leftClip, IClipPtr rightClip, const VideoCompositionParameters& parameters) override;
    ...
    void applyToAllPixels(wxImagePtr image, boost::function<float (int, int)> f) const;
    ...
    /// \param image on the opacity of this image the given method is applied
    /// \param f given x and y, the opacity of the image is multiplied by the resulting value of this method
    void applyToFirstLineThenCopy(wxImagePtr image, boost::function<float (int, int)> f) const;

    /// To be implemented by derived classes.
    /// When the image is fully opaque a more efficient implementation than iterating
    /// over all pixels may be possible.
    /// \param image on the opacity of this image the given method is applied
    virtual void handleFullyOpaqueImage(wxImagePtr image, boost::function<float (int, int)> f) const = 0;

    /// To be implemented by derived classes.
    /// The image contains alpha data. Therefore, an optimized implementation might
    /// not be possible.
    /// \param image on the opacity of this image the given method is applied
    virtual void handleImageWithAlpha(wxImagePtr image, boost::function<float (int, int)> f) const = 0;
    ...
    virtual boost::function<float (int,int)> getLeftMethod(wxImagePtr image, float factor) const = 0;
    ...
    /// \param image on the opacity of this image the given method is applied
    /// \param factor factor that indicates the progress of the transition 0.0 <= factor <= 1.0
    virtual boost::function<float (int,int)> getRightMethod(wxImagePtr image, float factor) const = 0;
};
