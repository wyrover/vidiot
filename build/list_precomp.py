...
#
# From: http://gamesfromwithin.com/the-care-and-feeding-of-pre-compiled-headers
#
# 1. Run a MSVC build with the option /showincludes enabled
#    (or a GCC build with -H)
# 2. Paste this python script into the dir which holds the build output
#    which should include a <library>.log log file.
# 3. cd to this dir
# 4. list_precomp.py <library>.log
...
def parse_includes(lines, ignore_path):
    ...
    include_regexp_vc  = re.compile('^\s*Note\: including file\:(\s+)(.*)')
