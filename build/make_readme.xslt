    ...
    <xsl:template match="log">
        ...
        <xsl:for-each select="logentry">
            ...
            <xsl:if test="@revision>1220">
                <xsl:choose>
                    <xsl:when test="@revision=1313"><xsl:text>&#xa;&#xa;Release 0.2.2 (revision </xsl:text><xsl:value-of select="@revision"/><xsl:text>)</xsl:text></xsl:when>
                    <xsl:when test="@revision=1310"><xsl:text>&#xa;&#xa;Release 0.2.1 (revision </xsl:text><xsl:value-of select="@revision"/><xsl:text>)</xsl:text></xsl:when>
