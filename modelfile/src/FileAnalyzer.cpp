...
FileAnalyzer::FileAnalyzer(wxStrings fileNames, wxWindow* parent)
    : mParent(parent)
	, mMostFrequentFrameRate(Config::get().ReadString(Config::sPathVideoDefaultFrameRate))
    , mMostFrequentAudioRate(std::make_pair(Config::get().ReadLong(Config::sPathAudioDefaultSampleRate), Config::get().ReadLong(Config::sPathAudioDefaultNumberOfChannels)))
    , mMostFrequentVideoSize(Config::get().ReadLong(Config::sPathVideoDefaultWidth), Config::get().ReadLong(Config::sPathVideoDefaultHeight))
