...
FileAnalyzer::FileAnalyzer(wxStrings fileNames, wxWindow* parent)
    : mParent(parent)
	, mMostFrequentFrameRate(Config::ReadString(Config::sPathVideoDefaultFrameRate))
    , mMostFrequentAudioRate(std::make_pair(Config::ReadLong(Config::sPathAudioDefaultSampleRate), Config::ReadLong(Config::sPathAudioDefaultNumberOfChannels)))
    , mMostFrequentVideoSize(Config::ReadLong(Config::sPathVideoDefaultWidth), Config::ReadLong(Config::sPathVideoDefaultHeight))
