...
model::Files ProjectViewAddFiles( wxFileNames paths, model::FolderPtr parent )
{
    ...
    for ( wxFileName path : paths )
    {
        model::NodePtrs nodes = util::thread::RunInMainReturning<model::NodePtrs>(std::bind(&model::Node::find, parent, path.GetLongPath())); // Check that long path is generated by vidiot
        ASSERT_EQUALS( nodes.size(), 1 );
        model::NodePtr node = nodes.front();
