...
model::FolderPtr FixtureGui::addAutoFolder( wxFileName path, model::FolderPtr parent )
{
    ...
    model::ProjectViewPtrs nodes = getRoot()->find( path.GetLongPath() ); // Converted to long path in vidiot
    ASSERT(nodes.size() == 1);
...
model::Files FixtureGui::addFiles( std::list<wxFileName> paths, model::FolderPtr parent )
{
    ...
    BOOST_FOREACH( wxFileName path, paths )
    {
        model::ProjectViewPtrs nodes = parent->find( path.GetLongPath() ); // Check that long path is generated by vidiot
        ASSERT(nodes.size() == 1);
        model::ProjectViewPtr node = nodes.front();
