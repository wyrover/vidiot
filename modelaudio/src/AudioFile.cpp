...
AudioChunkPtr AudioFile::getNextAudio(const AudioCompositionParameters& parameters)
{
    ...
    if (fileOpenFailed())
    {
        ...
        return boost::static_pointer_cast<AudioChunk>(boost::make_shared<EmptyChunk>(parameters.getNrChannels(), parameters.getChunkSize(), 0));
    }

    AudioChunkPtr audioChunk = boost::make_shared<AudioChunk>(static_cast<sample*>(0),parameters.getNrChannels(),parameters.getChunkSize(),0);
    samplecount nCombinedSamples = 0;

    while (nCombinedSamples < parameters.getChunkSize())
    {
        if (mPendingChunk)
        {
            // Continue with the 'leftover' from a previous getNext call
            ASSERT_NONZERO(mPendingChunk->getUnreadSampleCount());
        }
        else
        {
            ...
            else
            {
                ...
                if (mResampleContext == 0)
                {
                    ...
                    mPendingChunk = boost::make_shared<AudioChunk>(audioDecodeBuffer, parameters.getNrChannels(), nDecodedSamples, determinePts(nDecodedSamples));
                }
                else
                {
                    // Resample
                    ...
                    mPendingChunk = boost::make_shared<AudioChunk>(static_cast<sample*>(0), parameters.getNrChannels(), nExpectedOutputSamples, determinePts(nExpectedOutputSamples));
                    sample* resampled = mPendingChunk->getBuffer();
                    ...
                    mPendingChunk->setAdjustedLength(nResampledSamples);
        ...
        samplecount nRemainingSamples = parameters.getChunkSize() - nCombinedSamples;
        sample* output = audioChunk->getBuffer() + nCombinedSamples;

        if (mPendingChunk)
        {
            samplecount nCopySamples = std::min(mPendingChunk ->getUnreadSampleCount(), nRemainingSamples);

            memcpy(output,mPendingChunk->getUnreadSamples(),nCopySamples * AudioChunk::sBytesPerSample);

            mPendingChunk->read(nCopySamples);
            nCombinedSamples += nCopySamples;

            ASSERT_IMPLIES(mPendingChunk->getUnreadSampleCount() > 0, nCombinedSamples == parameters.getChunkSize());
            if (mPendingChunk->getUnreadSampleCount() == 0)
            {
                mPendingChunk.reset();
            }
        }
        ...
        {
            memset(output,0,nRemainingSamples * AudioChunk::sBytesPerSample);
            nCombinedSamples = parameters.getChunkSize();
        }
    }
