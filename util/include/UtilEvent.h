#ifndef UTIL_EVENT_GENERIC_H
#define UTIL_EVENT_GENERIC_H

#include <wx/event.h>

/**
* Implemented with a macro instead of a template, since I wasn't able
* to use the event enum in the macro. It isn't const at compile time,
* since it's generated by wxNewEventType().
*/

#define DECLARE_EVENT(enumname, classname, objecttype)\
    class classname : public wxEvent\
    {\
    public:\
        classname(objecttype value);\
        classname(const classname& other);\
        virtual wxEvent* Clone() const;\
        objecttype getValue() const;\
    private:\
        objecttype mValue;\
    };\
    wxDECLARE_EVENT(enumname, classname);

#define DEFINE_EVENT(enumname, classname, objecttype)\
    classname::classname(objecttype value)\
    :   wxEvent(wxID_ANY, enumname)\
    ,   mValue(value)\
    {}\
    classname::classname(const classname& other)\
    :   wxEvent(other)\
    ,   mValue(other.mValue)\
    {}\
    wxEvent* classname::Clone() const\
    {\
    return new classname(*this); \
    }\
    objecttype classname::getValue() const\
    {\
    return mValue;\
    }\
    wxDEFINE_EVENT(enumname, classname);\


//template <typename ValueType>
//class SingleValueEvent : public wxEvent
//{
//public:
//    SingleValueEvent(wxEventType eventtype, ValueType value)
//        :   wxEvent(wxID_ANY, eventtype)
//        ,   mValue(value)
//    {
//    }
//    SingleValueEvent(const SingleValueEvent& other)
//        :   wxEvent(other)
//        ,   mValue(other.mValue)
//    {
//    }
//    virtual wxEvent* Clone() const
//    {
//        return new SingleValueEvent(*this); 
//
//    }
//    ValueType getValue() const
//    {
//        return mValue;
//    }
//private:
//    ValueType mValue;
//};

#endif UTIL_EVENT_GENERIC_H

