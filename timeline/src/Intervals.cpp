...
void Intervals::addBeginMarker()
{
    ...
    mNewIntervalBegin = cursor + model::Convert::timeToPts(Config::get().read<double>(Config::sPathTimelineMarkerBeginAddition) * sSecond);
    mNewIntervalEnd = cursor + model::Convert::timeToPts(Config::get().read<double>(Config::sPathTimelineMarkerEndAddition)   * sSecond);
}
